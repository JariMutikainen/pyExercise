...
def add(*matricies):
    """
    Adds matricies to produce a sum matrix. 
    """
    def add_two_matricies(matrix1, matrix2):
        """
    ...
    for i in range(1,len(matricies)):
        if len(matricies[0]) != len(matricies[i]):
            raise ValueError("Given matrices are not the same size.")
        result = add_two_matricies(result,matricies[i])
...
#For the first bonus, modify your add function to accept and "add" any
#number of lists-of-lists. 

#print(add([[1, 9], [7, 3]], [[5, -4], [3, 3]], [[2, 3], [-3, 1]])) # [[8, 8], [7, 7]]
